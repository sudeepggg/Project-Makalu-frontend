...
const Pricing: React.FC = () => {
  ...
  const [displayMode, setDisplayMode] = useState<"overrides_only" | "all_products">("overrides_only");
  
  // State to control form modal popup visibility
  ...
  const handleSaved = () => {
    ...
    setRefreshTrigger((prev) => prev + 1); // Live refresh the list data table
  };
    ...
    <div className="space-y-6 relative">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setIsFormOpen(false); // Reset form if customer switches
              }}
              disabled={customersLoading}
            >
              <option value="">
                {customersLoading ? "Loading customer profiles..." : "Choose a customer profile..."}
              </option>
          ...
          <div>
            ...
            >
              ...
              <option value="all_products">
                Show All Master Products (Global Catalog With Overrides Placed Inline)
              </option>
            </select>
          </div>
        </div>

        {/* Action Button to trigger Form Modal */}
        {selectedCustomerId && (
      ...
      <div className="w-full">
        {selectedCustomerId ? (
          ...
          <div className="card p-12 text-center text-sm text-ink-faint border border-dashed border-surface-300 rounded-xl bg-surface-50/50">
            💡 Please choose a customer from the selection control field above to build and display their targeted pricing configurations.
          </div>
        )}
      </div>

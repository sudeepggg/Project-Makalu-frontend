...
const CustomerList: React.FC = () => {
  ...
  const { data: result, isLoading, isError } = useCustomers({ search: search || undefined, page });
  const customers = result?.data || [];
  const pagination = result?.pagination;

  if (isLoading) return <LoadingSpinner />;
  if (isError) return <div className="card p-4 text-red-600 text-sm">Error loading customers.</div>;
    ...
    <div className="card fade-in">
      ...
      <div className="overflow-x-auto">
        <table className="table-base">
          ...
          <tbody>
            {customers.length === 0 && (
              <tr><td colSpan={5} className="text-center text-ink-faint py-8">No customers found</td></tr>
            )}
            {customers.map((c: any) => (
              <tr key={c.id} onClick={() => setSelectedId(c.id)} className="cursor-pointer">
                ...
                <td className="font-mono text-sm">NPR {c.creditLimit?.toLocaleString()}</td>
                <td>
      ...
      {pagination && pagination.pages > 1 && (
        <div className="p-4 border-t border-surface-200 flex items-center justify-between text-sm text-ink-muted">
          ...
          <div className="flex gap-2">
            ...
            <button disabled={page >= pagination.pages} onClick={() => setPage(p => p + 1)} className="btn-secondary py-1 px-2 disabled:opacity-40">
